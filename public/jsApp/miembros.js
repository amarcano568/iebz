$(document).on("ready", function () {
    ...
    function listarMiembros() {
        ...
        objetoDataTables_Miembros = $("#tableMiembros").DataTable({
            ...
            language: {
                ...
                sZeroRecords:
                    "No se encontró ningun Miembro con la Condición del Filtro",
            ...
            .every(function () {
                ...
                var select = $(
                    '<select style="" id="' +
                        ...
                        '" data-placeholder="' +
                        placeholder +
                        '" class="form-control chosen-select"><option value=""></option></select>'
                )
    ...
    $("#body-miembros").on("click", "td.celda_de_descripcion", function () {
        ...
        //console.log(tr);
        var row = objetoDataTables_Miembros.row(tr);
    ...
    $("#btnAgregarMiembro").click(function () {
        ...
        $("#TituloModalReferencia").html(
            '<i class="text-primary fas fa-user-plus"></i> Agregar un nuevo Miembro..!'
        );
        $("#fotoUsuario").html(
            '<i class="img-thumbnail fa-8x far fa-user img-responsive avatar-view"></i> '
        );
        $("#adjuntosMiniatura").html(
            '<center><img src="img/documentos.png" height="100" width="100"><br><h3 class="text-center">No tiene Documentos Adjuntos...</h3></center>'
        );
        $("#grupoFamiliar").html(
            '<center><img src="img/familia.png" height="100" width="100"><br><h3 class="text-center">No tiene grupo Familiar...</h3></center>'
        );
        $("#home-tab").click();
    });
    ...
    $("#FormMiembro").validate({
        ...
        submitHandler: function (form) {
            ...
            if (idIglesia == "") {
                ...
                el.after(
                    $(
                        '<p class="errorDescripcion" style="color: #a94442;background-color: #f2dede;border-color: #ebccd1;padding:1px 20px 1px 20px;">Debe Selecionar una Iglesia</p>'
                    )
                );
            }
            ...
            alertify
                .confirm(
                    "Miembro",
                    '<h4 class="text-info">Esta seguro de guardar estos datos..?</h4>',
                    function () {
                        ...
                        $.ajax({
                            ...
                            .done(function (data) {
                                ...
                                if (data.success === true) {
                                    alertify.success(
                                        "Datos del Miembro actualizado...."
                                    );
                                } else {
                                    alertify.success(
                                        "Error - no se pudo actualizar los datos del Miembro..."
                                    );
                                }
                                $("#FormMiembro").each(function () {
                                    this.reset();
                    ...
                    function () {
                        // En caso de Cancelar
                        alertify.error(
                            "Se Cancelo el Proceso para Guardar los datos del Miembro."
                        );
                    }
                )
                .set("labels", {
    ...
    $("body").on("click", "#body-miembros a", function (e) {
        ...
        switch (accion_ok) {
            case "editarMiembro": // Edita Usuario
                ...
                $.ajax({
                    ...
                    .done(function (response) {
                        ...
                        $("#ModalAgregarMiembro").modal("show");
                        $("#TituloModalReferencia").html(
                            '<i class="fas fa-user-edit"></i> Editar datos del Miembro.'
                        );

                        $("#nombreCorto").text(response.data.nombre);
                        $("#profesionCorto").text(
                            response.data.profesionNombre
                        );

                        $("#idMiembro").val(response.data.id);
                        $("#idIglesia")
                            .val(response.data.idIglesia)
                        ...
                        $("#sexo")
                            .val(response.data.sexo)
                            .trigger("chosen:updated");
                        $("#codPostal").val(response.data.codigoPostal);
                        $("#direccion").val(response.data.direccion);
                        $("#comunidad")
                            .val(response.data.comunidad)
                            .trigger("chosen:updated");
                        listarProvincias(
                            response.data.comunidad,
                            response.data.provincia
                        );
                        $("#provincia")
                            .val(response.data.provincia)
                            .trigger("chosen:updated");
                        ...
                        $("#pais")
                            .val(response.data.paisNacimiento)
                            .trigger("chosen:updated");
                        $("#profesion")
                        ...
                        if (
                            ...
                            $("#fotoUsuario").html(
                                '<img class="img-thumbnail img-responsive avatar-view" src="img/fotos/' +
                                    response.data.foto +
            ...
            case "eliminarMiembro": // Eliminar Membro
                alertify
                    .confirm(
                        '<i class="text-danger far fa-trash-alt"></i><span class="text-danger"> Eliminar Miembro</span>',
                        '<h4 class="text-info">Esta seguro de eliminar la Ficha de <strong>' +
                            nombreMiembro +
                            '</strong> ..?</h4><br><center><img class="img-fluid img-thumbnail" src="' +
                            foto +
                        ...
                        function () {
                            // En caso de Cancelar
                            alertify.error(
                                '<i class="fa-2x fas fa-ban"></i><br>Se Cancelo el Proceso para Eliminar la ficha de ' +
                                    nombreMiembro
                            );
                        }
                    )
    ...
    $("#ModalAgregarFoto").on("shown.bs.modal", function () {
        iconoDropZone =
            '<br><br><i class="fa-4x fas fa-camera-retro"></i><br><h6>Click para agregar Foto.</h6>';
    ...
    function configuraDropZone(iconoDropZone) {
        ...
        $("#formDropZone").html("");
        $("#formDropZone").append(
            "<form action='subir-foto' method='POST' files='true' enctype='multipart/form-data' id='dZUpload' class='dropzone borde-dropzone' style='width: 100%;padding: 0;cursor: pointer;'>" +
    ...
    $(document).on("click", ".foto-miembro", function (event) {
        ...
        if (fileImage == "img\\fotos\\") {
            ...
            alertify.error(
                '<i class="fa-2x fas fa-photo-video"></i><br>Este miembro no tiene foto cargada.'
            );
            return;
        }
    ...
    $(document).on("click", ".clickPicture", function (event) {
        ...
        $("#nameFileDetalle").html(
            '<h6><i class="fas fa-image"></i> ' + nameShort + "</h6>"
        );
        $("#dateFileDetalle").html(
            '<h6><i class="far fa-calendar-alt"></i> ' + nameDate + "</h6>"
        );
        ...
        $("#ModalDetalleImagen").modal("show");
        imagen =
            '<img id="imgZoom" data-zoom-image="' +
            fileImage +
            '" style="height: 23em;display: block;width: 100%" src="' +
            fileImage +
            '" alt="image" />';
        $("#divDetalleImagen").html(imagen);
        $("#imgZoom").elevateZoom({
    ...
    $("#ModalAgregarDocumento").on("shown.bs.modal", function () {
        iconoDropZone =
            '<br><br><i class="fa-4x far fa-image"></i><br><h6>Click para agregar Documento.</h6>';
    ...
    function configuraDropZoneDocumento(iconoDropZone) {
        ...
        $("#formDropZoneDocumento").append(
            "<form action='subir-documento' method='POST' files='true' enctype='multipart/form-data' id='dZUpload' class='dropzone borde-dropzone' style='width: 100%;padding: 0;cursor: pointer;'>" +
                "<div style='padding: 0;margin-top: 0em;' class='dz-default dz-message text-center'>" +
                iconoDropZone +
                "</div></form>"
        );
    ...
    $(document).on("click", ".clickDelete", function (event) {
        ...
        alertify
            .confirm(
                '<i class="text-danger far fa-trash-alt"></i><span class="text-danger"> Eliminar Documento</span>',
                '<h4 class="text-info">Esta seguro de Eliminar este Documento..?</h4><br><center><img src="' +
                    archivo +
                    '" height="100" width="100"></center>',
                function () {
                    $.ajax({
                ...
                function () {
                    // En caso de Cancelar
                    alertify.error(
                        "Se Cancelo el Proceso para Eliminar el Documento."
                    );
                }
            )
            .set("labels", {
    ...
    $("#btnAgregarRelacion").click(function () {
        ...
        if (miembroToRelacion == "") {
            alertify.set("notifier", "position", "top-center");
            alertify.error(
                "Debe selecionar un miembro para agregarlo al grupo familiar."
            );
        ...
        if (parentesco == "") {
            alertify.set("notifier", "position", "top-center");
            alertify.error(
                "Debe selecionar un parentesco para el miembro seleccionado."
            );
            $("#parentesco").focus();
            return;
        }
    ...
    $(document).on("click", "#BtnNuevo", function (event) {
        ...
        $("#tituloModal").html(
            '<i class="fas fa-user-edit"></i> Nueva Profesión.'
        );
    ...
    $("#FormMantProfesion").validate({
        ...
        messages: {
            nombreProfesion:
                '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Debe introducir Descripción para el cargo.',
        },
        submitHandler: function (form) {
            ...
            $.ajax({
                ...
                .done(function (data) {
                    ...
                    $("#profesion").append(
                        '<option value="' +
                            data.idNuevaProfesion +
                            '">' +
                            nom +
                            "</option>"
                    );
                    $("#profesion")
                        .val(data.idNuevaProfesion)
                        .trigger("chosen:updated");
